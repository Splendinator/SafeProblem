...
public:
	...
	void printKey(Safe<NUMLOCKS, 4> *s, int numSafes) throw(IOException);
	
	//void printLockedSafe(const Safe<5, 4> &s) throw(IOException);
	
	void printLockedSafe(Safe<NUMLOCKS, 4> *s, int numSafes) throw(IOException);

	void printMultiSafe(Safe<NUMLOCKS, 4> *safes, int numSafes) throw(IOException);

	int readLockedSafe(Safe<NUMLOCKS, 4> *safe) throw(IOException);
